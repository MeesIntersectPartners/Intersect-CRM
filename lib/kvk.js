...
async function zoekBedrijven(gemeente, pagina = 1, sbiCode = null, naam = null) {
  try {
    const params = {
      type: 'hoofdvestiging',
    ...
    if (naam) params.naam = naam; // KvK ondersteunt naam als zoekparameter
    ...
    console.error(`[KvK] Fout bij zoeken${gemeente?' in '+gemeente:''}${naam?' naam:'+naam:''}:`, err.message);
    return null;
  }
}
