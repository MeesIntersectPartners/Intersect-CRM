...
async function zoekBedrijven(gemeente, pagina = 1, sbiCode = null) {
  try {
    const params = {
      type: 'hoofdvestiging',
    ...
    // Gemeente is optioneel — als null zoeken we heel Nederland op SBI
    if (gemeente) params.plaats = gemeente;
    // KvK API ondersteunt sbiCode als zoekfilter
    if (sbiCode) params.sbiCode = sbiCode;
    ...
    console.error(`[KvK] Fout bij zoeken${gemeente?' in '+gemeente:''}${sbiCode?' SBI:'+sbiCode:''}:`, err.message);
    return null;
  }
}
