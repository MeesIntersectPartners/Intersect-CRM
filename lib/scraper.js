...
async function scrapeWebsite(website) {
  ...
  for (const pad of TEAM_PADEN) {
    ...
    if (contacten.length) {
      ...
      // Pak direct email van teampagina als die er is
      const teamEmails = (teamHtml.match(EMAIL_REGEX) || []).filter(isDirectEmail);
      if (teamEmails.length) {
        resultaat.directEmail = teamEmails[0];
      }
      break;
    }
  }
